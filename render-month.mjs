...
//show calendar
export const renderCalendar = (date) => {
	// add already rendered data as dom attribute
	calendarContainer.setAttribute("data-renderedDate", date);
	// call helper to show month and day slots directly into existing container
	showCalendar(date.getMonth(), calendarContainer); // Now starting Month is Current Month - 
	// but first day, for now, starts same as was for June - Sunday
};
